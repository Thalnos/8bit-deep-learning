...
with open('plotdata.csv', 'rb') as csvfile:
	...
	for i in range(17):
		...
		if i == 15:
			train32 = np.array(row[:6], dtype = 'float')
		if i == 16:
			test32 = np.array(row[:6], dtype = 'float')
				
	
	x16 = range(3,9)
	x8 = range(3,9)
	x32 = range(3,9)
	bench32fl = 89.7200006246567
	plt.figure(1)
	ax = plt.subplot(211)	
	
	plt.plot(x8, train8, linewidth=2, label='8 bit')
	plt.plot(x16, train16, linewidth=2, label='16 bit')
	plt.plot(x32, train32, linewidth=2, label='32 bit')
