...
# easier to read.
# CNN = 160362 weights
def main(train = True, model='cnn', num_epochs=50, bits = (8,16), precision=((2,),(6,)), precisionchange = 2):
    dataloc = "../quantize/Models/MNIST3/"
    # Load the dataset
    print("Loading data...")
...
#						print(np.asarray(output.eval()))
						if layernr == 1 or layernr == 3 or layernr == 4:
							output = np.asarray(np.round((np.asarray(output.eval()) % (1 << bit)) * (1.0 / (1 << prec))), dtype=theano.config.floatX)
							output = theano.shared(output)
						elif layernr == 7:
							output = np.asarray(np.round((np.asarray(output.eval()) % (1 << bit)) * (1.0 / (1 << (prec + precisionchange)))), dtype=theano.config.floatX)
							output = theano.shared(output)
						layernr += 1
...
#					print("layersend: ", time.time())
					...
					for index, out  in np.ndenumerate(lim_outputs):
						if out >= (1 << bit):
							...
							out = -((1 << bit) - 1)
						if out >= 0:
							lim_outputs[index] = np.round((out % (1 << bit)) * (1.0 / (1 << prec - precisionchange)))
						else:
							lim_outputs[index] = -(np.round((-out % (1 << bit)) * (1.0 / (1 << prec - precisionchange)))) 
